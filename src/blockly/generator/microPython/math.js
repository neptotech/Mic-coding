...
/**
 * @fileoverview Generating.MicroPython code for the Math blocks.
 *
...
const LoadMathCodeGenerator = () => {
     ...
     * (X ^ Y).
     *.MicroPython code: loop { X operator Y }
     * @param {!Blockly.Block} block Block to generate the code from.
     * @return {array} Completed code with order of operation.
     */
     ...
     *.MicroPython code: loop { operator(X) }
     * @param {!Blockly.Block} block Block to generate the code from.
     * @return {array} Completed code with order of operation.
     */
     ...
     * INFINITY).
     *.MicroPython code: loop { constant }
     * TODO: Might need to include "#define _USE_MATH_DEFINES"
     *       The.MicroPython header file already includes math.h
     * @param {!Blockly.Block} block Block to generate the code from.
     ...
     *.MicroPython code: complex code, can create external functions.
     * @param {!Blockly.Block} block Block to generate the code from.
     * @return {array} Completed code with order of operation.
     */
     ...
     *.MicroPython code: loop { (X < Y ? Y : ( X > Z ? Z : X)) }
     * @param {!Blockly.Block} block Block to generate the code from.
     * @return {array} Completed code with order of operation.
     */
     ...
     *.MicroPython code: loop { math_random_int(X, Y); }
    ...
    Blockly.MicroPython['math_random_seed'] = function (block) {
        var seed = Blockly.MicroPython.valueToCode(block, 'SEED',
            Blockly.MicroPython.ORDER_NONE) || '0';
        var argument1 = Blockly.MicroPython.valueToCode(block, 'TO',
            Blockly.MicroPython.ORDER_NONE) || '0';
        var functionName = Blockly.MicroPython.variableDB_.getDistinctName(
     ...
     *.MicroPython code: loop { (rand() / RAND_MAX) }
     * @param {!Blockly.Block} block Block to generate the code from.
     * @return {string} Completed code.
     */
    ...
    Blockly.MicroPython['math_max_min'] = function (block) {
        var operator = block.getFieldValue('OP');

        var arg1 = Blockly.MicroPython.valueToCode(block, 'NUM1', Blockly.MicroPython.ORDER_NONE) || '0';
        var arg2 = Blockly.MicroPython.valueToCode(block, 'NUM2', Blockly.MicroPython.ORDER_NONE) || '0';

        var code = `${operator.toLowerCase()}(${arg1}, ${arg2})`;
