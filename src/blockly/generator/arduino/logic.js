...
const LoadLogicCodeGenerator = () => {
    ...
    Blockly.Arduino['switch_statement'] = function (block) {
        var variable_name = Blockly.Arduino.variableDB_.getName(block.getFieldValue('switch_var'), Blockly.Variables.NAME_TYPE);
        var code = "switch (" + variable_name + ") {\n";
        var argument, branch;
        for (var n = 0; n <= block.caseCount_; n++) {
            argument = Blockly.Arduino.valueToCode(block, 'CASE' + n,
                Blockly.Arduino.ORDER_NONE) || n;
            branch = Blockly.Arduino.statementToCode(block, 'DO' + n);
            code += "  case " + argument + ":\n" + branch + "  break;\n";
        }
...
export default LoadLogicCodeGenerator;
