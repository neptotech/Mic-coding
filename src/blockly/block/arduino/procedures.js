        ...
        setArgsType: function (existingVars) {
            ...
            for (var name in existingVars) {
                for (var i = 0, length_ = varNames.length; i < length_; i++) {
                    if (name === varNames[i]) {
                        if (existingVars[name].typeId != 'Undefined') {
                            this.argsTypes[name] = existingVars[name];
                        }
                    } else {
                        const varNameForUser = Blockly.Arduino.variableDB_.getNameForUserVariable_(name);
                        if (varNameForUser === varNames[i]) {
                            this.argsTypes[varNameForUser] = existingVars[name];
                        }
                    }
                }
            }
...
export default LoadProcedureBlocks;
