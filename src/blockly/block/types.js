...
const LoadTypes = () => {
  /** Single character. */
  Blockly.Types.VOID = new Blockly.Type({
    typeId: 'Void',
    typeMsgName: 'ARD_TYPE_VOID',
    compatibleTypes: []
  });
  ...
  Blockly.Types.getTypeFromText = function (text) {
    for (let typeKey in Blockly.Types) {
      if (typeKey === text) {
  ...
  Blockly.Types.getChildBlockType = function (block) {
    ...
    while (nextBlock && (nextBlock.getBlockType === undefined) &&
      ...
      if (nextBlock.inputList[0].connection) {
