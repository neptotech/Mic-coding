...
MIDI_RESPONSE[REPORT_VERSION] = function (board) {
    console.log('REPORT_VERSION=>', board.firmware.name);
...
SYSEX_RESPONSE[MICRODUINO_MOTION] = function (board) {
    const address = (board.currentBuffer[4] & 0x7F) | ((board.currentBuffer[5] & 0x7F) << 7);
    const register = (board.currentBuffer[6] & 0x7F) | ((board.currentBuffer[7] & 0x7F) << 7);
    let value = address | register << 8;
    board.emit('acceleration', board.currentBuffer[3] ? -value : value);
...
function ideaBoardKeyParser(address, register, reply) {
    if (address === 0x56 && register === 0x09) {
        ...
        for (let j = 0; j < 2; j++) {
            ...
            // 事件发生
            if (lastValue !== temp) {
                ideaBoardKey[j].value = (temp === 1) ? 1 : 0;
...
function Board(port) {
    ...
    this.transport.on('data', data => {
        ...
        for (byte of data) {
            ...
            if (first === START_SYSEX && last === END_SYSEX) {
                if (this.currentBuffer[1] === QUERY_FIRMWARE) {
                    this.emit('QUERY_FIRMWARE');
                }
    ...
    async function ready() {
        ...
        await board.init();
        await board.reset();
        board.emit('ready', board.firmware);
    }
...
Board.prototype.reportVersion = function () {
...
Board.prototype.queryFirmware = function () {
...
Board.prototype.pwmWriteForRotate = function (mode, pin, value) {
    const data = []
    data.push(ANALOG_MESSAGE | pin, value & 0x7F, (value >> 7) & 0x7F)
...
Board.prototype.servoRotate = function (pin, value) {
    if (this && this.firmware && this.firmware.name &&
        (this.firmware.name.startsWith('sp') || this.firmware.name.startsWith('Bu'))) {
        if (pin >= 14 && pin <= 21) {
            pin = pin + 10
        }
    }
...
let avrLEDModel = ''
Board.prototype.checkAvrLEDModel = function (model) {
    let result = avrLEDModel === Number(model)
    if (!result) avrLEDModel = Number(model)
    return result
}
 ...
 * @param {string} pinModel The pin to read data from
 * @param {function} callback The function to call when data has been received
 */

Board.prototype.digitalRead = function (pin, pinModel, callback) {
    if (this && this.firmware && this.firmware.name &&
        (this.firmware.name.startsWith('sp') || this.firmware.name.startsWith('Bu'))) {
        if (pin >= 14 && pin <= 21) {
            pin = pin + 10
    ...
    if (this.pins && this.pins.length > pin) {
        if (this.pins[pin].mode !== this.MODES.INPUT) {
            return new Promise((resolve, reject) => {
                ...
                this.pinMode(pin, pinModel ? this.MODES.PULLUP : this.MODES.INPUT)
                // report digital
                if (this && this.firmware && this.firmware.name &&
                    (this.firmware.name.startsWith('sp') || this.firmware.name.startsWith('Bu'))) {
...
Board.prototype.motorStop = function () {
    const msg = [START_SYSEX, MICRODUINO_MOTOR, 0, 3, END_SYSEX]
    return writeToTransport(this, msg)
}
...
Board.prototype.buzzerNote = function (pin, frequency, beats) {
    return this.buzzerControl(pin, frequency, beats)
}
...
Board.prototype.buzzerControl = function (pin, freq, duration) {
    ...
    const msg = [START_SYSEX, MICRODUINO_BUZZER, pin, freq & 0x7F, (freq >> 7) & 0x7F, (duration * 1000) & 0x7F, ((duration * 1000) >> 7) & 0x7F, END_SYSEX]
    return writeToTransport(this, msg).then((res) => new Promise((resolve) => setTimeout(() => {
        resolve(res)
    }, duration * 1000)))
}

Board.prototype.buzzerSong = function (pin, songNumber, wait) {
    ...
    for (let i = 0; i < music[songNumber].length; i++) {
        ...
        if (note === 0) {
            p = p.then(_ => this.buzzerOff(pin).then(_ => new Promise(resolve => setTimeout(resolve, duration / 1000))))
        } else {
            if (this && this.firmware && this.firmware.name &&
                (this.firmware.name.startsWith('sp') || this.firmware.name.startsWith('Bu'))) {
                if (pin >= 14 && pin <= 21) {
            ...
            p = p.then(_ => this.buzzerSongPlay(pin, note, duration / 1000))
        }
    }
...
Board.prototype.buzzerSongPlay = function (pin, freq, duration) {
    ...
    return this.buzzerControl(pin, freq, duration)
}

Board.prototype.acceleration = async function (axes, status) {
    const msg = [START_SYSEX, MICRODUINO_MOTION, status, (status ? Number(axes) : 0x00), END_SYSEX]
    const p = new Promise((resolve) => {
        this.once(`acceleration`, (data) => {
            resolve(data)
        })
    })
...
Board.prototype.irKey = function (pin, key, event) {
...
Board.prototype.touchKey = function (pin, event, pinModel) {
    if (this && this.firmware && this.firmware.name &&
        (this.firmware.name.startsWith('sp') || this.firmware.name.startsWith('Bu'))) {
        if (pin >= 14 && pin <= 21) {
            pin = pin + 10
    ...
    return this.digitalRead(pin, pinModel).then(value => {
        let result = false
...
let wordStock = [
    ...
    [0x00, 0x00, 0x00, 0x02, 0x01, 0x02, 0x01, 0x04]
]

function getWordStock(str) {
    let dotMaxtrixArr = []
    for (let index of str) {
        let char = wordStock[index.toString().charCodeAt() - 32]
        for (let [i, index1] of char.entries()) {
            let str = index1.toString(2)
            let str1 = (new Array(8 - str.length).fill('0')).join('')
            str1 += str
            if (i >= 7) {
                dotMaxtrixArr.push('00000000')
                continue
            }
            if (str1 !== '00000000') {
                dotMaxtrixArr.push(str1)
            }
        }
    }
    return dotMaxtrixArr
}

function checkDotMaxtrixArr(dotMaxtrixArr, color) {
    let result = ''
    let resultLength = 0
    if (dotMaxtrixArr.length - n < 8) n = 0
    dotMaxtrixArr = _.drop(dotMaxtrixArr, n)
    dotMaxtrixArr = _.dropRight(dotMaxtrixArr, dotMaxtrixArr.length - 8)
    if (dotMaxtrixArr.length < 8) {
        let length = dotMaxtrixArr.length
        for (let i = 0; i < 8 - length; i++) dotMaxtrixArr.push('00000000')
    }
    for (let i = 7; i >= 0; i--) {
        result += _.map(dotMaxtrixArr, i).join('')
        resultLength++
        if (resultLength === 8) break
    }
    return result.replace((new RegExp('0', 'g')), '#000000').replace((new RegExp('1', 'g')), color || '#00FF00')
}

function setMatrixPoint(x, y, target, cmd, isMonochromatic) {
    dotMaxtrixBuffer[y * 8 + x] = target
    let coordinate = isMonochromatic ? 0b10000000 : 0b11000000
    coordinate |= x << 3
    coordinate |= y
    cmd.push(coordinate)
...
Board.prototype.clearDotMatrixColor = function (addr) {
    dotMaxtrixBuffer = null
    n = 0
    dotMaxtrixText = ''
    clearInterval(time)
    return this.i2cRequestWrite(addr, addr === 48 ? [0xC0, 0x00] : [0x60, 0x00])
}

Board.prototype.dotMatrixText = async function (addr, text, color) {
    let self = this
    let result = getWordStock(text)
    if (result.length > 8) {
        result = (new Array(8).fill('00000000').concat(result).concat(new Array(8).fill('00000000')))
    } else {
        let num = parseInt((8 - result.length) / 2)
        result = (new Array(num).fill('00000000').concat(result).concat(new Array(8 - result.length - num).fill('00000000')))
    }
    if (dotMaxtrixText !== text || (color && dotMaxtrixColor !== color)) {
        if (time) {
            clearInterval(time)
        }
        if (n === 0) this.clearDotMatrixColor(+addr)
        time = setInterval(function () {
            dotMaxtrixColor = color
            self.dotMatrixColor(+addr, checkDotMaxtrixArr(result, color), addr === 48)
            n++
        }, 300)
        dotMaxtrixText = text
    }
...
Board.prototype.dotMatrixColor = function (addr, colors, isMonochromatic, clearTime) {
    if (time && clearTime) {
        dotMaxtrixText = ''
        n = 0
        clearInterval(time)
    ...
    if (!dotMaxtrixBuffer) {
        dotMaxtrixBuffer = new Array(64).fill('#000000')
        if (!isMonochromatic) cmd.push(0x60, 0x00) // clear screen
    }
    let darkScreen = true
    ...
    for (let y = 0; y < 8; y++) {
        for (let x = 0; x < 8; x++) {
            ...
            if (buffer !== target) {
                setMatrixPoint(x, y, target, cmd, isMonochromatic)
            }
        }
    }
    if (darkScreen) {
        cmd = [(isMonochromatic ? 0xC0 : 0x60)]
    }
...
Board.prototype.bot_car_backup = function (direction, speed) {
    let msg = []
    if (direction.toString() === '3') {
        msg = [START_SYSEX, 0x0C, 1, 3, 0, 0, 0 & 0x7F, (0 >> 7) & 0x7F, speed & 0x7F, (speed >> 7) & 0x7F, END_SYSEX]
    } else {
        msg = [START_SYSEX, 0x0C, 1, 3, 0, 0, speed & 0x7F, (speed >> 7) & 0x7F, 0 & 0x7F, (0 >> 7) & 0x7F, END_SYSEX]
    }
    return writeToTransport(this, msg)
}
...
Board.prototype.bot_buzzer_song = function (songNumber, wait) {
    buzzerSongStop = false
    const song = music[songNumber]
    const beats = rhythm[songNumber]
    let p = Promise.resolve()
...
Board.prototype.bot_lego_motor = function (pin, speed) {
    let forward = 1
    if (speed < 0) {
        speed = speed * -1
        forward = 0
    }
    const msg = [START_SYSEX, 0x06, pin, forward, speed & 0x7F, (speed >> 7) & 0x7F, END_SYSEX]
    return writeToTransport(this, msg)
}
let n = 0
let time
...
Board.prototype.pingRead = function (opts, callback) {
    if (!this.pins[opts.pin].supportedModes.includes(PING_READ)) {
