...
const LOOKUP_PORT_START = 5741;
const LOOKUP_PORT_END = 5742;
...
export default class SocketDaemon extends Daemon {
  ...
  compile = (fqbn, device, code, cb) => {
    if (this.agentFound.getValue()) {
      fetch(`${this.pluginURL}/compile`, {
        method: 'POST',
        body: JSON.stringify({
          port: device,
          code,
      ...
      .then((value) => {
        console.log('[socket-daemon][compile] value:', value);
        return value.json();
      })
      .then((data) => {
        console.log('[socket-daemon][compile] data:', data);
        if (cb) {
          cb({
            data,
            result: true
      ...
      .catch((err) => {
        console.error('[socket-daemon][compile] err:', err);
        if (cb) {
          cb({result: false, error: err});
        }
      });
    }
  }
